   ...
   @include('bbp_prod1.bbp_prod1_menu')
      ...
      <div class="br-pagebody">
        <div class="br-section-wrapper">
          <h6 class="br-section-label">Ordenar Productos</h6>
          <p class="br-section-text">Permite ordenar numericamente Banners en página inicial.  <span style="color: #ff0000;" ng-cloak ng-show="orderok"><b><i class="fa fa-check-square" aria-hidden="true"></i> Listo, el orden fue actualizado! Presiona F5 para ver resultados</b></span></p>
            ...
            <table class="table mg-b-0  table-colored table-dark">
              ...
              <tbody>
                ...
                <tr>
                 ...
                 <td style="<?=$bgcolor;?>">
                      <a href="{{route('bbp-prod1-delete-pro',[$reg->bbp_prod1_id,$reg->bbp_prod1_token])}}"><button type="button" class="btn btn-danger btn-sm">Elim</button></a></td>
                  
                  <td style="<?=$bgcolor;?>">{{$reg->bbp_prod1_id}}</td>
                 
                  <td style="<?=$bgcolor;?>">{{$reg->bbp_prod1_product}}</td>                  
                  
                  <td style="<?=$bgcolor;?>"><?if(isset($reg->bbp_prod1_image1)){?><img src="{{url('/')}}/storage/uploaddir/{{$reg->bbp_prod1_image1}}" style="height:80px;" /><?}?></td>
                  ...
                  <td style="<?=$bgcolor;?>">{{$reg->bbp_prod1_order}}</td> 
                  
                  <td style="<?=$bgcolor;?>">                     

                        <select name="order" ng-model="order{{$reg->bbp_prod1_id}}" ng-change="OrderAR('{{$reg->bbp_prod1_id}}','{{$reg->bbp_prod1_token}}',this.order{{$reg->bbp_prod1_id}})" data-placeholder="Ordenar">
